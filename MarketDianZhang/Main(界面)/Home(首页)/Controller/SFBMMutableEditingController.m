//
//  SFBMMutableEditingController.m
//  MarketDianZhang
//
...
@interface SFBMMutableEditingController ()<UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate, UIImagePickerControllerDelegate, RichTextEditDelegate, MMRichEditAccessoryViewDelegate,UITextViewDelegate,ImagePickerDelegate,CWVoiceViewDelegate>
{
    NSInteger _selectColor;
    NSString *_selectTime;
}
//@property (weak, nonatomic) IBOutlet NSLayoutConstraint *viewWide;
...
@implementation SFBMMutableEditingController
...
#pragma mark--CWVoiceViewDelegate
...
//    [self showCurrentImageInImgeview:image];
    [self handleInsertImageFromVoice:image voicePath:filePath];//如果添加录音，从这个方法开始添加上录音的地址，
}
...
- (void)selectPhoto {
    if (@available(iOS 11, *)) {
        [UIScrollView appearance].contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentAutomatic; //iOS11 解决SafeArea的问题，同时能解决pop时上级页面scrollView抖动的问题,在选中图片或退出相册时：再重新设置为UIScrollViewContentInsetAdjustmentNever
    }
    // 手机相册选择
    UIImagePickerController *imagePickerController = [UIImagePickerController new];
    imagePickerController.delegate = self;
...
- (void)rightBarBtnSelect:(UIButton *)button
{
    ...
    if (titleStr.length == 0) {
        [self showTheMostCommonAlertmessage:@"请输入标题"];
        return;
    }
    ...
    [muDic setValue:colorStr forKey:@"color"];//文章标识颜色
    [muDic setValue:titleStr forKey:@"title"];//标题
    [muDic setValue:htmlStr forKey:@"content"];//正文内容
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MYNSLog(@"_titleModel.textContent=%@",_titleModel.textContent);
    MYNSLog(@"_datas=%@",_datas);
    MYNSLog(@"_datas[indexPath.row]=%@",_datas[indexPath.row]);
