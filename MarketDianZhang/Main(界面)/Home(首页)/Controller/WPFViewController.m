...
//        }
//    }
    for (MyContactListModel2 *pe in personArray)
    {
        @autoreleasepool{
            NSString *name = pe.name;
            NSString *idS = pe.ID;
            NSString *company = pe.company;
            NSData *data = pe.headpath;

            //i就是将来需要查询具体对象的id
//            [WPFPinYinDataManager addInitializeString:name identifer:idS];
            [WPFPinYinDataManager addInitializeString:name identifer:idS company:company headImageData:data];
...
//    cell = [tableView dequeueReusableCellWithIdentifier:@"SFBMHomeViewControllerCell0"];
    MyContactListModel2 *people = self.dataSource[indexPath.row];
//    cell.textLabel.text = people.name;
    cell.cellTitleL.text = people.name;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyContactListModel2 *p = [self.dataSource objectAtIndex:indexPath.row];
    VC.model = p;//假数据
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"WPFPeopleCell" bundle:nil] forCellReuseIdentifier:@"WPFPeopleCell"];
