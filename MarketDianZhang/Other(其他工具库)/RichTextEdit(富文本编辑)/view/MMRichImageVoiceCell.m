...
@interface MMRichImageVoiceCell () <MMTextViewDelegate, UITextViewDelegate>
@property (nonatomic, strong) MMTextView* textView;
@property (nonatomic, strong) UIProgressView *progressView;
@property (nonatomic, strong) UIButton *reloadButton;
@property (nonatomic, strong) MMRichImageVoiceModel* imageModel;
@end

@implementation MMRichImageVoiceCell
...
- (void)setupUI {
    ...
    [self.reloadButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.equalTo(@(convertLength(250)));\
        make.height.equalTo(@(convertLength(300)));
...
- (void)updateWithData:(id)data {
    if ([data isKindOfClass:[MMRichImageVoiceModel class]])
    {
        MMRichImageVoiceModel* imageModel = (MMRichImageVoiceModel*)data;
        // 设置旧的数据delegate为nil
...
//        _imageModel.uploadDelegate = self;
        ...
        self.reloadButton.hidden = YES;//隐藏上传按钮 目前废弃
...
- (UIButton *)reloadButton {
    if (!_reloadButton) {
        ...
        [_reloadButton setImage:[UIImage imageNamed:@"voice_btn_stop"] forState:UIControlStateNormal];
