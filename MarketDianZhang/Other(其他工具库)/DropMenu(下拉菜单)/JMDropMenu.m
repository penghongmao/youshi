...
- (instancetype)initWithFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr ImageArr:(NSArray *)imageArr Type:(JMDropMenuType)type LayoutType:(JMDropMenuLayoutType)layoutType RowHeight:(CGFloat)rowHeight Delegate:(id<JMDropMenuDelegate>)delegate {
    if (self = [super initWithFrame:frame]) {
        ...
        if (type == JMDropMenuTypeWeChat) {
            self.RGBStrValueArr = [NSMutableArray arrayWithObjects:@(26),@(42),@(55), nil];
            _titleColor = [UIColor whiteColor];
            _lineColor = [UIColor colorWithRed:(float)42/255 green:(float)60/255 blue:(float)73/255 alpha:1];
        } else {
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        if (_type == JMDropMenuTypeWeChat) {
            _tableView.backgroundColor = [UIColor colorWithRed:26 / 255.0 green:42 / 255.0 blue:55 / 255.0 alpha:1];
        } else {
...
- (void)layoutSubviews {
    ...
    self.line1.frame = CGRectMake(3, self.contentView.frame.size.height - 0.5, self.contentView.frame.size.width, 0.5);
}
...
//        self.line1.backgroundColor = [UIColor colorWithRed:(float)42/255 green:(float)60/255 blue:(float)73/255 alpha:1];
    }
