...
- (BOOL)initAudioRecorder
{
    ...
    //音频质量,采样质量 AVAudioQualityMin最低质量省空间，如果需要高质量的录音 此处需要调整
    [recordSettings setValue:[NSNumber numberWithInt:AVAudioQualityMin] forKey:AVEncoderAudioQualityKey];
    
    // 3. 创建录音对象
    NSError *error = nil;
    _audioRecorder = [[AVAudioRecorder alloc] initWithURL:url settings:recordSettings error:&error];
    _audioRecorder.meteringEnabled = YES;// 是否启用音频测量，默认为NO，一旦启用音频测量可以通过updateMeters方法更新测量值
    if (error) {
        NSLog(@"error--创建录音对象出错了--%@",error);
        return NO;
    }
    return YES;
}
...
// 判断麦克风权限！！！！！！建议：先判断权限，权限允许了再执行开始录音代码
