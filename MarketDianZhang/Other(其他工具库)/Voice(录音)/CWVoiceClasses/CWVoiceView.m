...
@interface CWVoiceView ()<UIScrollViewDelegate,CWRecordViewDelegate>
...
//        scrollV.contentSize = CGSizeMake(self.cw_width * 3, 0);
        scrollV.contentSize = CGSizeMake(self.cw_width * 1, 0);
...
{
    NSLog(@"filePath3-mao==%@",filePath);
    if (self.delegate && [self.delegate respondsToSelector:@selector(playSendVoiceFile2:)]) {
        [self.delegate playSendVoiceFile2:filePath];
    }
}
-(void)cancleSendVoiceFile1:(NSString *)filePath
{
    NSLog(@"filePath3-hong==%@",filePath);
    if (self.delegate && [self.delegate respondsToSelector:@selector(cancleSendVoiceFile2:)]) {
        [self.delegate cancleSendVoiceFile2:filePath];
    }
}
@end
