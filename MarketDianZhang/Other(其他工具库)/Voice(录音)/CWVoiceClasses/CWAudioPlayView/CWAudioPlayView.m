...
//    NSLog(@"%@",btn.titleLabel.text);
    ...
    if (btn == self.sendButton) { // 发送
        NSLog(@"发送录音...path0: %@",[CWRecordModel shareInstance].path);
        if (self.delegate && [self.delegate respondsToSelector:@selector(playSendVoiceFile:)]) {
            [self.delegate playSendVoiceFile:[CWRecordModel shareInstance].path];
        }
    }else {
        NSLog(@"取消发送并删除录音");
        [[CWRecorder shareInstance] deleteRecord];
        if (self.delegate && [self.delegate respondsToSelector:@selector(cancleSendVoiceFile:)]) {
            [self.delegate cancleSendVoiceFile:[CWRecordModel shareInstance].path];
        }
    }
